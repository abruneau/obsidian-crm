import { GroupElement, Grouping, Groupings, Literal, Literals } from "@blacksmithgu/datacore";
import { CSSProperties, Fragment, isValidElement, ReactNode, RefObject, useContext, useMemo, useRef } from "react";
import { CURRENT_FILE_CONTEXT, Lit } from "../markdown";
import { ControlledPager, useDatacorePaging } from "../paging";

interface MarkdownListItemLike {
    $text?: string;
    $file?: string;
}
...
export interface ListViewProps<T> {
     ...
     */
    renderer?: (row: T) => ReactNode | Literal;
...
function ensureListElement<T>(element: T): ReactNode {
...
function DefaultListElement<T>({ element }: { element: T }) {
    ...
    // Check if element has MarkdownListItem properties instead of using instanceof
    if (element && typeof element === 'object' && '$text' in element && '$file' in element) {
        const listItem = element as MarkdownListItemLike;
        return <Lit inline={true} value={listItem.$text} sourcePath={listItem.$file} />;
    }
