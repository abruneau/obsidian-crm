...
import { ReactNode, RefObject, useContext, useMemo, useRef } from "react";
import { ControlledPager, useDatacorePaging } from "../paging";
import { CURRENT_FILE_CONTEXT, Lit } from "../markdown";
import { useAsElement, useInterning } from "../hooks";
import "./table.css";
...
export function TableView<T>(props: TableViewProps<T>) {
    ...
    const paging = useDatacorePaging({
        ...
        container: tableRef as RefObject<HTMLElement>,
    });
            ...
            <table className="datacore-table">
                ...
                <tbody>
                    {pagedRows.map((row, index) => (
                        <VanillaRowGroup key={index} level={0} groupings={groupings} columns={columns} element={row} />
                    ))}
                </tbody>
            </table>
            {paging.enabled && (
        ...
        return (
            <>
                ...
                {element.rows.map((row, index) => (
                    <VanillaRowGroup key={index} level={level + 1} columns={columns} element={row} />
                ))}
            </>
        );
    } else {
