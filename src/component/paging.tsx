...
function RawControlledPager({
	...
	const visiblePages = useMemo(
		() => splitPages(realPage, totalPages),
		[realPage, totalPages]
	);
	...
	return (
		<div className="dc-paging-control">
			...
			{visiblePages.map((pages, i) => (
				<div key={`page-group-${i}`}>
					{i > 0 && (
						<button key={`separator-${i}`}
						...
						>
							<svg
								...
								viewBox="0 0 24 24"
								width="1em"
								className="dc-paging-control-ellipsis"
							>
								<circle
									cx="12"
									cy="12"
							...
							<svg
								...
								viewBox="0 0 24 24"
								width="1em"
								className={`dc-paging-control-leap-${
									isLeftSeperator(
										realPage,
										visiblePages.length,
										i
...
export const ControlledPager = memo(RawControlledPager);
...
export function usePaging({
	...
	const [pageStart, setPageStart] = useState(() =>
		clamp(initialPage, 0, totalPages - 1)
	);

	const setBoundedPage = useCallback(
		(page: number) =>
			setPageStart(clamp(page, 0, totalPages - 1) * pageSize),
		[pageSize, totalPages]
	);
...
export function useDatacorePaging({
	...
	const pageSize =
		typeof paging === "number" ? paging : settings.defaultPageSize;
	const pagingEnabled = typeof paging === "number" || paging === true;
	const shouldScroll =
		(typeof scrollOnPageChange === "number" &&
			scrollOnPageChange >= pageSize) ||
		!!(scrollOnPageChange ?? settings.scrollOnPageChange);

	const [page, totalPages, rawSetPage] = usePaging({
		initialPage,
		pageSize,
		elements,
	});
 ...
 * @hidden
 */
function isLeftSeperator(
	realPage: number,
	visiblePagesLength: number,
